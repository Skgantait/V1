...
if (isset($_GET['Logout'])){
    if (isset($_SESSION['calgoricadmincore'])){
        $_SESSION['calgoricadmincore'] = array();
        unset($_SESSION['calgoricadmincore']);
...
if(isset($_POST["LogSubmit"])){
    ...
    if ($count > 0){
        if (md5($conpass) == $rtrn['pass']){
                
            $_SESSION['adminlgin']=$rtrn;
            $otp = get_OTP();
            $_SESSION['adminlgin']['otp'] = $otp;

            if (sendOTP($rtrn['email'], $rtrn['email'], $otp)){
                $regisStp = 1;
            } else{
                $_SESSION['adminlgin'] = array();
                unset($_SESSION['adminlgin']);
    ...
    $regisStp = 1;
    if (!isset($_SESSION['adminlgin'])){
        $regisStp = 0;
        $msg = "Error in submit.";
    } else {
        $email = $_SESSION['adminlgin']['email'];
        $otp = trim(strip_tags($_POST["otp"]));
        //echo $otp . "\n";
        //echo $_SESSION['adminlgin']['otp'] . "\n";
        //die();

        if ($otp == $_SESSION['adminlgin']['otp']){
            $_SESSION['calgoricadmincore']=$_SESSION['adminlgin']['flag'];
            $_SESSION['adminlgin'] = array();
            unset($_SESSION['adminlgin']);
            echo '<script>window.parent.location.href = "index.php";</script>';
            exit();
    ...
    if (isset($_GET['step'])){
        ...
        if ($tm == 2){
            if (isset($_SESSION['adminlgin'])){
                ...
                if (sendOTP($_SESSION['adminlgin']['email'], $_SESSION['adminlgin']['email'], $otp)){
                    $regisStp = 1;
                } else{
                    $_SESSION['adminlgin'] = array();
                    unset($_SESSION['adminlgin']);
