...
export class FuncbodyComponent implements OnInit {
  ...
  insertPost() {
    ...
    this.funcService.insert(data).subscribe({
      next: (res: FuncResponse) => {
        if (res.error) {
      ...
      error: (err) => {
        const msg = err?.error?.message || 'Insert failed. Please try again.';
  ...
  updatePost() {
    const data = this.buildPostData();
    this.funcService.update(data).subscribe({
      next: (res: FuncResponse) => {
        if (res.error) {
          this.toast.error(res.message);
        } else {
